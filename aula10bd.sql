...
--1 crie uma view chamada v_depcidade que liste o nome de cada departamento com o nome da cidade onde este departamento está 
--localizado.
--Após a criacao executar Select * from v_depcidade; retorna 36 linhas
create view v_depcidade as select nome(cidade) as cidade, nome(departamento) as departamento from tbl_cidades as cidade 
inner join tbl_departamentos as departamento on departamento.cod_cidade = cidade.cod_cidade
select * from v_depcidade

--2 crie uma view denominada v_depcidadehouston, a partir de v_depcidade que mostre somente os departamentos 
--localizados em Houston
-- execute select * from v_depcidadehouston
create view v_depcidadehouston as select * from v_depcidade where cidade = 'Houston'
...
create view v_opsalario as select sum(salario), avg(salario) from tbl_empregados
...
create view v_empregados_salarial as select nome, salario from tbl_empregados
where salario > 5500
...
create view v_departemp(departamento,quantidade) as select nome(departamento), count(nome(empregados)) from tbl_departamentos as departamento
inner join tbl_empregados as empregados on departamento.cod_departamento = empregados.cod_departamento
...
create view v_departrouble (departamento) as select nome(departamentos) from tbl_departamentos as departamentos
left join tbl_empregados as empregados on departamentos.cod_departamento = empregados.cod_departamento
left join tbl_projetos as projeto on departamentos.cod_departamento = projeto.cod_departamento
group by nome(departamentos)
having count(projeto. *) > count(empregados. *)
